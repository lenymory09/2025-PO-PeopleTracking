...
def generate_detections(frame: np.ndarray) -> Tuple[List[Tuple], List[Boxes]]:
    ...
    results = model(frame, classes=[0], device="cpu", conf=0.5, iou=0.5, verbose=False)[0]
    detections = []
    boxes = results.boxes
...
class PersonTracker:
    tracked_persons: Dict[int, List[np.ndarray]] = {}
    ...
    def get_frames(self):
        ...
        for camera in self.cameras:
            frames.append(camera.track_people(self.tracked_persons))
        return frames

    def start(self) -> NoReturn:
        while True:
            for i, camera in enumerate(self.cameras, start=1):
                frame_analysed = camera.track_people(self.tracked_persons)
...
class EnhancedPersonTracker:
    def __init__(self, sources):
        ...
        torchreid.utils.load_pretrained_weights(self.reid_model, 'models/osnet_ain_duke.pth.tar-60')
    ...
    def get_nearest_person_improved(self, embed: np.ndarray, assigned_ids: List[int]):
        """Improved matching with multiple s
        trategies"""
        ...
        for known_id, person in self.tracked_persons.items():
            ...
            # Combined score
            score = 0.7 * cosine_dist + 0.3 * (euclidean_dist / 10.0)
            
            # Use best match from recent embeddings
            if score < DISTANCE_THRESHOLD and score < best_score:
                    best_score = score
                    matched_id = known_id
    ...
    def generate_embeddings(self, frame: np.ndarray, boxes, crops: List[np.ndarray] = []):
        #return self.feature_extractor(crops)
        result = []
        for box in boxes:
    ...
    def extract_features(self, bgr_imgs):
        ...
        with torch.no_grad():
            ...
            feats = self.re(batch_t).cpu().numpy()
...
class Camera:
    def __init__(self, source: str, ai: str, reid: EnhancedPersonTracker):
        self.model = ai
        self.source = source
        self.cap = cv2.VideoCapture(source)
        self.reid = reid  # Référence au tracker principal
        self.frame_count = 0
    ...
    def process_id(self, embed: np.ndarray, matched_id: Optional[int],
        ...
        if matched_id:
            ...
            self.reid._update_gallery(matched_id, embed)
            #self.tracker.tracked_persons[matched_id].append(embed)
            ...
            matched_id = self.reid._create_id(embed)
    ...
    def track_people(self) -> Optional[np.ndarray]:
        ...
        embeds = self.reid.generate_embeddings(frame, detections, crops=[])
